...
ChainKit 是多鏈 SDK 解決方案。Tron 是第一條支援的鏈。目標發布到 NuGet 作為開源套件。

### 設計原則

- **分層架構**：使用者按需引用，只要離線簽章就引 Core，不用拉整個 SDK
...
├── ChainKit.Tron.Core/              → 離線功能（地址、金鑰、簽章、ABI 編碼）
├── ChainKit.Tron.Protocol/          → Protobuf 定義、交易構建與序列化
├── ChainKit.Tron.Client/            → Provider 抽象 + HTTP/gRPC 實作
└── ChainKit.Tron/                   → 高階 Facade + TRC20 合約管理 + 交易監聽

tests/
├── ChainKit.Core.Tests/
├── ChainKit.Tron.Core.Tests/
├── ChainKit.Tron.Protocol.Tests/
├── ChainKit.Tron.Client.Tests/
└── ChainKit.Tron.Tests/
```
...
```
ChainKit.Tron → ChainKit.Tron.Client → ChainKit.Tron.Protocol → ChainKit.Tron.Core → ChainKit.Core
```

每層只依賴下層，不反向依賴。未來新增其他鏈遵循相同結構：`ChainKit.Ethereum.Core → ChainKit.Core`。
...
| 套件 | 版本 | 用途 | 所在層 | 授權 |
|------|------|------|--------|------|
| NBitcoin.Secp256k1 | 3.2.0 | ECDSA secp256k1 簽章 | Tron.Core | MIT |
| NBitcoin | 9.0.5 | BIP39 助記詞 / BIP44 HD 推導 | Tron.Core | MIT |
| Google.Protobuf | 3.34.1 | Protobuf 序列化 | Tron.Protocol | BSD |
| Grpc.Net.Client | 2.76.0 | gRPC 客戶端 | Tron.Client | MIT |
| Grpc.Tools | 2.80.0 | .proto 編譯（build only） | Tron.Protocol | Apache 2.0 |
| NetMQ | 4.0.2.2 | ZMQ 監聽 | Tron | MPL 2.0 |
| Keccak-256 | 自行實作 | 地址 / ABI 雜湊 | Tron.Core | — |
...
- **不用 Secp256k1.Net**：需要 native binary，不利於 NuGet 套件分發
...
## 第二層：ChainKit.Tron.Core — 離線功能

純計算，零網路依賴。依賴 ChainKit.Core + NBitcoin + NBitcoin.Secp256k1。
...
## 第三層：ChainKit.Tron.Protocol — 交易構建

依賴 ChainKit.Tron.Core + Google.Protobuf。Tron 官方 `.proto` 編譯成 C# 類別。
...
## 第四層：ChainKit.Tron.Client — Provider 抽象

依賴 ChainKit.Tron.Protocol + Grpc.Net.Client。
...
## 第五層：ChainKit.Tron — 高階 Facade

依賴所有下層。提供一步到位的高階 API，內部整合多個低階呼叫。
...
public record TronTransactionDetail(
    ...
    // --- 狀態（需要 Full Node + Solidity Node 配合判斷）---
...
public record Trc20ReceivedEventArgs(
    string TxId, string FromAddress, string ToAddress,
    string ContractAddress, decimal Amount,
...
| 層級 | 測試方式 | 說明 |
|------|----------|------|
| Core | 單元測試 | 已知測試向量（BIP39 標準向量、secp256k1 簽章向量），100% 離線 |
| Protocol | 單元測試 | 交易構建 + Protobuf 序列化，用已知交易驗證 |
| Client | Mock 測試 | Mock HTTP/gRPC 回應，驗證 Provider 解析邏輯 |
| Tron | 整合測試 | Mock Provider，測試高階 API 的整合邏輯（多步驟串接） |
| Tron | E2E 測試 | 連 Nile 測試網，標記 `[Category("Integration")]`，CI 可選擇跳過 |
