...
特點：
- 金額使用 `decimal`，單位為 **TRX**（`10m` = 10 TRX）
...
特點：
- 金額使用 `long`，單位為 **Sun**（`1_000_000` Sun = 1 TRX）
- 需要自行處理 ref block 設定、簽章、廣播
- 可能會 throw exception
...
if (result.Success)
{
    ...
    if (detail.TokenTransfer is not null)
    {
        ...
        Console.WriteLine($"原始金額:   {token.RawAmount}");     // 最小單位
        Console.WriteLine($"可讀金額:   {token.Amount}");        // 轉換後的值，可能為 null
    }
...
```csharp
// 只查 TRX 餘額
var result = await client.GetBalanceAsync("TYourAddress...", Array.Empty<string>());
...
var result = await client.GetBalanceAsync(
    ...
    new[]
    {
        ...
        "TEkxiTehnzSmSe2XqrBj4w32RUN966rdz8",    // 另一個 TRC20 代幣
    });
...
if (result.Success)
{
    ...
    foreach (var (contract, trc20) in balance.Trc20Balances)
    {
        ...
        Console.WriteLine($"  原始餘額: {trc20.RawBalance}");   // 最小單位
...
// 查詢代幣基本資訊
var nameResult = await trc20.NameAsync();
var symbolResult = await trc20.SymbolAsync();
var decimalsResult = await trc20.DecimalsAsync();
var supplyResult = await trc20.TotalSupplyAsync();
...
// 查詢餘額
var balanceResult = await trc20.BalanceOfAsync("TTargetAddress...");
if (balanceResult.Success)
    Console.WriteLine($"餘額: {balanceResult.Data}");

// 查詢授權額度
var allowanceResult = await trc20.AllowanceAsync(
    owner: "TOwnerAddress...",
...
var result = await trc20.TransferAsync(
...
// 授權另一個地址使用你的代幣
var result = await trc20.ApproveAsync(
    spender: "TSpenderAddress...",
    amount: 1000m);
...
// 鑄造新代幣（需要 minter 權限）
var mintResult = await trc20.MintAsync(
    to: "TRecipientAddress...",
    amount: 10000m);

// 銷毀自己的代幣
var burnResult = await trc20.BurnAsync(amount: 500m);

// 銷毀他人的代幣（需要 allowance）
var burnFromResult = await trc20.BurnFromAsync(
    from: "TFromAddress...",
    amount: 200m);
```
...
watcher.OnTrc20Received += (sender, e) =>
{
    ...
    Console.WriteLine($"  原始金額: {e.RawAmount}");          // 最小單位（一定有值）
...
SDK 中涉及代幣金額的地方都同時提供兩種格式：

| 欄位 | 類型 | 說明 | 永遠有值？ |
|------|------|------|-----------|
| `RawAmount` / `RawBalance` | `decimal` | 鏈上原始值（最小單位） | 是 |
| `Amount` / `Balance` | `decimal?` | 經 decimals 轉換後的人類可讀值 | 否（decimals 未知時為 null） |

```csharp
// 範例：USDT（6 decimals）
// 鏈上值 = 1000000
// RawAmount = 1000000m    （一定有值）
// Amount    = 1.0m        （轉換後：1000000 / 10^6）
...
`TronClient`、`TronHttpProvider`、`TronGrpcProvider`、`Trc20Contract` 都實作 `IDisposable`，`TronTransactionWatcher` 實作 `IAsyncDisposable`。

```csharp
// 方式 1：using 語句
using var provider = new TronHttpProvider(TronNetwork.Mainnet);
using var client = new TronClient(provider);
// client dispose 時會同時 dispose provider
